...
当前正式位置：**定位 + 视觉理论专项 / M03 / Day018 — GNSS / RTK / Timestamp / Latency / Synchronization / Calibration**。Day008–Day017 已完成并有正式讲义。**M02 Module Graduation Exam 尚未完成，保留为待回补，不宣告 M02 模块毕业。** 用户继续 M03 Day16–19，完成后仍按专项进入 M05 Day22–26；M04 Simulation 暂不作为当前专项前置主线。`docs/lessons/` 当前没有 Day001–Day007 的正式学习记录，因此这里不把它们标成已完成。
