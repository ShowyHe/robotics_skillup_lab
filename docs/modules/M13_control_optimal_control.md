...
1. 今日目标：理解控制器通过feedback持续纠偏，而不是只“给一个速度”。
2. 前置：M12 state-space/dynamics。
3. 必须教学：open/closed loop；`e=r-y`；P/I/D；steady-state error；overshoot/oscillation；discrete PID；sample time；saturation/rate limit；integral windup/anti-windup；derivative noise；command vs actual actuator input。
4. 深度：Feedback/PID L4。
...
3. 必须教学：equilibrium `f(x*,u*)=0`；对LTI系统或非线性系统在equilibrium附近的local linearization，continuous `x_dot=Aδx` 中 `Re(λ)<0` 表示局部指数衰减，discrete `δx_(k+1)=A_dδx_k` 中 `|λ|<1` 表示局部离散稳定；不能脱离线性/局部线性语境泛化；pole概念；complex mode/oscillation；rise/overshoot/settling/steady-state error；state feedback `u=-Kx`；closed-loop `A-BK`；stability≠performance；delay对stability的影响；Lyapunov quadratic intuition只做认识。
...
1. 今日目标：回答“系统能不能被输入驱动到需要的状态”和“state能不能从measurement中恢复”。
2. 前置：M09 observability + M12 state-space。
3. 必须教学：`x_dot=Ax+Bu, y=Cx+Du`；controllability；控制能矩阵 `𝒞=[B,AB,...,A^(n-1)B]`；`rank(𝒞)=n`；physical coupling；uncontrollable unstable mode；observability；可观矩阵 `𝒪=[C;CA;...;CA^(n-1)]`；`rank(𝒪)=n`；unobservable mode；actuator/sensor数量≠rank；state feedback/pole placement概念；LQR/MPC前提关系。
...
1. 今日目标：理解state error与control effort的trade-off。
...
2. 前置：Day83。
3. 必须教学：prediction sequence；horizon N与`T=NΔt`；stage/terminal cost；state/input constraints；speed/accel/steering limits；receding horizon；execute first action；re-optimize；horizon/dt trade-off；feasibility；warm start；MPC vs LQR。
4. 深度：Receding Horizon/Constraints L4。
5. 工程连接：local navigation/manipulation trajectory。
6. 不展开：QP/NMPC solver细节。
7. 考核：N、dt、T与constraint推理。
8. 毕业考点：MPC、Horizon、Constraints。

# Day85 — MPPI / Sampling / Rollout / Weight / Update
1. 今日目标：理解MPPI优化的是future control sequence而非global path。
...
2. 前置：Day80–87。
3. 必须教学：path vs trajectory；lateral/heading/velocity error；actuator dynamics；velocity/acceleration saturation；model mismatch；sensor→estimate→planning→control→actuator latency；state age；delay-induced oscillation；feedback source/timestamp；controller frequency≠freshness；smoothing trade-off；safety override；prediction vs actual feedback；evidence chain。
...
2. 前置：Day80–88。
3. 必须教学：Planner vs Controller；state source/frame/time/freshness；model/dt/limits；sampling/horizon/std/warm-start；critic raw/weighted cost；clipping/smoothing/downstream limiter；actual feedback；behavior→candidate cause；E-stop recovery；MPPI源码定位 state/noise/rollout/model/critic/cost/weight/update/shift/constraint/publish；算法本体 vs Nav2接口 vs 公司额外策略；regression metrics。
4. 深度：Source Mapping/Failure Attribution L5。
5. 工程连接：公司MPPI + Nav2官方MPPI。
6. 不展开：新增控制算法。
7. 考核：给源码/log定位MPPI阶段和behavior root cause。
...
## 30% 核心基础
硬门槛：feedback、PID、local linear stability/eigenvalue、controllability/observability、LQR cost、MPC receding horizon/constraints、MPPI rollout-cost-weight-update、warm start、latency/saturation。

## 50% 综合系统场景
至少覆盖：`A-BK` stability；MPC horizon/constraint；MPPI weight/update简算；急停command≠actual state；stale LIO导致S形摆动；窄路critic/std/warm-start/horizon综合分析。

## 20% Source / Formula / Design
在公司MPPI和Nav2官方MPPI中定位state、motion model、sampling、rollout、critic、weight、update、warm start、clipping、feedback、publish；公司源码语义以实际读取为准。

## 通过标准
总分≥85%；必须明确 `Planner Path ≠ Predicted Trajectory ≠ Controller Command ≠ Actual Motion`；不能把旧command当真实state；所有稳定性结论必须说明线性/LTI或局部线性化适用语境。
